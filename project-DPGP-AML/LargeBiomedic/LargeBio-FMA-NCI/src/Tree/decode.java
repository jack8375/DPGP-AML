...
public class decode {
    public static List<double[][]> decodeBT(ArrayList<LinkedTreeNode> linkedTree, int num) throws IOException {
        ...
        int[][] size = definedMatrix.readArrayInt(definedMatrix.cluster_Size_FMA_NCI, 70, 2);
        ...
        for (int i = linkedTree.size() - 1; i >= 0; i--) {
            if (linkedTree.get(i).getStr().equals("NGram")) {
                linkedTree.get(i).setValue(definedMatrix.readArrayFromFile("D:\\Biomed\\LargeBio-FMA-NCI\\block_matrix\\NGram\\NGram_" + (num + 1), size[num][0], size[num][1]));
            }
            if (linkedTree.get(i).getStr().equals("SMOA")) {
                linkedTree.get(i).setValue(definedMatrix.readArrayFromFile("D:\\Biomed\\LargeBio-FMA-NCI\\block_matrix\\SMOA\\SMOA_" + (num + 1), size[num][0], size[num][1]));
            }
            if (linkedTree.get(i).getStr().equals("dictionary")) {
                linkedTree.get(i).setValue(definedMatrix.readArrayFromFile("D:\\Biomed\\LargeBio-FMA-NCI\\block_matrix\\Dictionary\\dictionary_" + (num + 1), size[num][0], size[num][1]));
            }
