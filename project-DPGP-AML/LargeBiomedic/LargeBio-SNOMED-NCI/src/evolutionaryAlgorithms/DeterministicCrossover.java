...
public class DeterministicCrossover implements Serializable{
    ...
    //确定性交叉
    public static List<ArrayList<LinkedTreeNode>> deterministicCrossover(List<ArrayList<LinkedTreeNode>> populationBetter, int selectionSize, List<String> PSA,
                                                                         List<List<String>> sourceCluster, List<List<String>> targetCluster, int taskNum, double crossoverProbability) throws IOException {

        double[] fitnessOfPopulation = Fitness.fitnessOfPopulation(populationBetter, PSA, sourceCluster, targetCluster, taskNum);
        for (int i = 0; i < populationBetter.size() / 2; i++) {

            ArrayList<LinkedTreeNode> parentInd1 = parentInd(populationBetter, selectionSize, fitnessOfPopulation);
            ...
            ArrayList<LinkedTreeNode> parentInd2 = parentInd(populationBetter, selectionSize, fitnessOfPopulation1);
            ...
            double parentInd1Fitness = Fitness.getFitness(parentCopyInd1, PSA, sourceCluster, targetCluster, taskNum);
            double parentInd2Fitness = Fitness.getFitness(parentCopyInd2, PSA, sourceCluster, targetCluster, taskNum);
            List<ArrayList<LinkedTreeNode>> offspring = crossover(parentInd1, parentInd2, crossoverProbability);
            ArrayList<LinkedTreeNode> offspring1 = offspring.get(0);
            ArrayList<LinkedTreeNode> offspring2 = offspring.get(1);
            double offspring1Fitness = Fitness.getFitness(offspring1, PSA, sourceCluster, targetCluster, taskNum);
            double offspring2Fitness = Fitness.getFitness(offspring2, PSA, sourceCluster, targetCluster, taskNum);
            //System.out.println(offspring1Fitness);
            //System.out.println(offspring2Fitness);

            double semSim1 = semSim(offspring1, parentCopyInd1, taskNum);
            double semSim2 = semSim(offspring1, parentCopyInd2, taskNum);
            double semSim3 = semSim(offspring2, parentCopyInd1, taskNum);
            double semSim4 = semSim(offspring2, parentCopyInd2, taskNum);
            ...
            /*for (int j = 0; j < fitnessOfPopulation.length; j++) {
                System.out.println(fitnessOfPopulation[j]);
            }
            System.out.println("----------------------------");*/
        }
        //System.out.println("已完成" + (populationBetter.size() / 2) + "次种群交叉");
        return populationBetter;
    }

    //交叉操作
    public static List<ArrayList<LinkedTreeNode>> crossover(ArrayList<LinkedTreeNode> parentInd1, ArrayList<LinkedTreeNode> parentInd2, double crossoverProbability) {

        List<ArrayList<LinkedTreeNode>> offspring = new ArrayList<>();
        int n1, n2;
        while (true) {
            n1 = Double.valueOf(Math.random() * parentInd1.size()).intValue();  //随机选择一个finction节点
            ...
            if ((parentInd1.get(n1).getStr().equals("add") || parentInd1.get(n1).getStr().equals("sub") || parentInd1.get(n1).getStr().equals("mul") || parentInd1.get(n1).getStr().equals("div")
                    || parentInd1.get(n1).getStr().equals("max") || parentInd1.get(n1).getStr().equals("min") || parentInd1.get(n1).getStr().equals("avg")) && rand1 < crossoverProbability)
                break;
        }
        while (true) {
            ...
            if ((parentInd2.get(n2).getStr().equals("add") || parentInd2.get(n2).getStr().equals("sub") || parentInd2.get(n2).getStr().equals("mul") || parentInd2.get(n2).getStr().equals("div")
                    || parentInd2.get(n2).getStr().equals("max") || parentInd2.get(n2).getStr().equals("min") || parentInd2.get(n2).getStr().equals("avg")) && rand2 < crossoverProbability)
                break;
        }
        LinkedTreeNode node1 = parentInd1.get(n1);
        LinkedTreeNode node2 = parentInd2.get(n2);
        LinkedTreeNode nodeCross = new LinkedTreeNode();  //用于交换的中间节点
    ...
    //计算个体间的语义相似度
    public static double semSim(ArrayList<LinkedTreeNode> ind1, ArrayList<LinkedTreeNode> ind2, int taskNum) throws IOException {
        List<double[][]> binaryAndSimilarityMatrix1 = decode.decodeBT(ind1, taskNum);
        double[][] binaryMatrix1 = binaryAndSimilarityMatrix1.get(0);
        List<double[][]> binaryAndSimilarityMatrix2 = decode.decodeBT(ind2, taskNum);
    ...
    //通过选拔，获取待交叉的parent
    public static ArrayList<LinkedTreeNode> parentInd(List<ArrayList<LinkedTreeNode>> population, int selectionSize, double[] fitnessOfPopulation) throws IOException {
        ...
        /*for (int i = 0; i < fitnessOfSubPopulation1.length; i++) {
            System.out.println(fitnessOfSubPopulation1[i]);
        }
        System.out.println("----------------------------");*/
        ...
        /*for (int i = 0; i < fitnessOfSubPopulation2.length; i++) {
            System.out.println(fitnessOfSubPopulation2[i]);
        }
        System.out.println("----------------------------");*/
        int maxIndex1 = 0;
    ...
    public static int indToIndex(ArrayList<LinkedTreeNode> ind, List<ArrayList<LinkedTreeNode>> population) {
        ...
        for (; i < population.size(); i++) {
            if (population.get(i).get(0) == ind.get(0))  //比较根节点的地址
                break;
        }
        return i;
    }

    //轮盘赌选择子种群(不能重复选择个体，防止破坏种群多样性)
    public static List<ArrayList<LinkedTreeNode>> roulette(List<ArrayList<LinkedTreeNode>> population, int selectionSize, double[] fitnessOfPopulation) {
        ...
        double[] copyFitness1 = new double[fitnessOfPopulation.length];
        for (int i = 0; i < copyFitness1.length; i++) {
            copyFitness1[i] = fitnessOfPopulation[i];
        }
        for (int i = 0; i < selectionSize; i++) {
            double r = Math.random();
            double[] probabilityOfFitness = new double[copyFitness1.length];
            double sum = 0;
            for (int j = 0; j < copyFitness1.length; j++) {
                sum += copyFitness1[j];
            }
            double acc = 0;
            for (int j = 0; j < copyFitness1.length; j++) {
            ...
            for (int index = 0; index < probabilityOfFitness.length; index++) {
                if (r <= probabilityOfFitness[index]) {
                    ...
                    copyFitness1[index] = 0;
                    break;
                }
            }
        }
        //long endTime = System.currentTimeMillis();
        //System.out.println("轮盘赌运行时间： " + (endTime - startTime) + "ms");
    ...
    public static ArrayList<LinkedTreeNode> copy(ArrayList<LinkedTreeNode> list) {
        ...
        for (int i = 1; i < copy.size(); i++) {
            root = copy.get(i);
            if (!root.getStr().equals("NGram") && !root.getStr().equals("SMOA") && !root.getStr().equals("dictionary")) {
    ...
    //用于复制一个linkedTree对象（序列化和反序列化的方法）
    public static LinkedTreeNode copyTree(LinkedTreeNode root) throws IOException, ClassNotFoundException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ObjectOutputStream oos = new ObjectOutputStream(baos);
        oos.writeObject(root); // 对Company对象进行序列化
        oos.close();
        byte[] bytes = baos.toByteArray(); // 获取Company对象序列化后的字节数组
        ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
        ObjectInputStream ois = new ObjectInputStream(bais);
        LinkedTreeNode newTree = (LinkedTreeNode) ois.readObject(); // 对Company对象进行反序列化
        ois.close();
        return newTree;
    }
}
