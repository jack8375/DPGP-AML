...
public class PopulationDiversityEnhancement {
    ...
    public static void diversityEnhancement(List<ArrayList<LinkedTreeNode>> optimaPopulation, List<ArrayList<LinkedTreeNode>> otherPopulation,
                                            int neighborhoodSize, int SPsize, int taskNum) throws IOException {
        List<ArrayList<LinkedTreeNode>> unionPopulation = new ArrayList<>();
        int[] indexArr1 = crowdnessSort1(optimaPopulation, neighborhoodSize, taskNum);
        int m = indexArr1.length - 1;
        for (int i = 0; i < SPsize; i++) {
            unionPopulation.add(optimaPopulation.get(indexArr1[m]));
            m--;
        }
        int[] indexArr2 = crowdnessSort2(otherPopulation, neighborhoodSize, taskNum);
        int n = 0;
        for (int i = 0; i < SPsize; i++) {
        ...
        int[] indexArr3 = crowdnessSort2(unionPopulation, neighborhoodSize, taskNum);
        int p = 0;
    ...
    //拥挤度排序后的索引(选择拥挤个体)
    public static int[] crowdnessSort1(List<ArrayList<LinkedTreeNode>> population, int neighborhoodSize, int taskNum) throws IOException {
        double[] crowdnessArr = new double[population.size()];
        for (int i = 0; i < population.size(); i++) {
            crowdnessArr[i] = crowdness1(i, population, neighborhoodSize, taskNum);
        }
        return indexSort(crowdnessArr);
    }

    //拥挤度排序后的索引(选择疏松个体)
    public static int[] crowdnessSort2(List<ArrayList<LinkedTreeNode>> population, int neighborhoodSize, int taskNum) throws IOException {
        double[] crowdnessArr = new double[population.size()];
        for (int i = 0; i < population.size(); i++) {
            crowdnessArr[i] = crowdness2(i, population, neighborhoodSize, taskNum);
        }
        return indexSort(crowdnessArr);
    }

    //计算个体的拥挤度(选择拥挤个体)
    public static double crowdness1(int indIndex, List<ArrayList<LinkedTreeNode>> population, int neighborhoodSize, int taskNum) throws IOException {
        double[] semSimArr = new double[population.size() - 1];
        for (int i = 0; i < population.size(); i++) {
            if (i < indIndex)
                semSimArr[i] = DeterministicCrossover.semSim(population.get(indIndex), population.get(i), taskNum);
            if (i == indIndex)
                continue;
            if (i > indIndex)
                semSimArr[i - 1] = DeterministicCrossover.semSim(population.get(indIndex), population.get(i), taskNum);
        }
    ...
    //计算个体的拥挤度(选择疏松个体)
    public static double crowdness2(int indIndex, List<ArrayList<LinkedTreeNode>> population, int neighborhoodSize, int taskNum) throws IOException {
        double[] semSimArr = new double[population.size() - 1];
        for (int i = 0; i < population.size(); i++) {
            if (i < indIndex)
                semSimArr[i] = DeterministicCrossover.semSim(population.get(indIndex), population.get(i), taskNum);
