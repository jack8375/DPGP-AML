...
public class PopulationDiversityEnhancement {
    ...
    public static double crowdness1(int indIndex, List<ArrayList<LinkedTreeNode>> population, int neighborhoodSize, List<List<String>> source, List<List<String>> target) throws IOException {
        ...
        for (int i = 0; i < population.size(); i++) {
            if (i < indIndex)
                semSimArr[i] = DeterministicCrossoverDP.semSim(population.get(indIndex), population.get(i), source, target);
            if (i == indIndex)
                continue;
            if (i > indIndex)
                semSimArr[i - 1] = DeterministicCrossoverDP.semSim(population.get(indIndex), population.get(i), source, target);
        }
