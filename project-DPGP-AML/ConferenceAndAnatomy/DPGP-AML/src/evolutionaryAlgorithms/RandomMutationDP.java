...
public class RandomMutationDP {

    //变异操作
    public static List<ArrayList<LinkedTreeNode>> mutation(List<ArrayList<LinkedTreeNode>> populationWorse, int selectionSize, List<String> PSA, List<List<String>> source, List<List<String>> target, double mutationProbability) throws IOException {
        List<ArrayList<LinkedTreeNode>> populationUnion = new ArrayList<>();
        for (int i = 0; i < populationWorse.size(); i++) {
            populationUnion.add(DeterministicCrossoverDP.copy(populationWorse.get(i)));
        }
        ...
        List<ArrayList<LinkedTreeNode>> populationUpdate = DeterministicCrossoverDP.roulette(populationUnion, selectionSize, Fitness.fitnessOfPopulation(populationUnion, PSA, source, target));
    ...
    public static ArrayList<LinkedTreeNode> mutationWay2(ArrayList<LinkedTreeNode> linkedTree, String[] str1, String[] str2, String[] str3, double mutationProbability) {
        List<ArrayList<LinkedTreeNode>> list = DeterministicCrossoverDP.crossover(linkedTree, BT.createTree(str1, str2, str3, 3), mutationProbability);
        return list.get(0);
    }
