...
public class decode {
    public static List<double[][]> decodeBT(ArrayList<LinkedTreeNode> linkedTree, List<List<String>> source, List<List<String>> target) throws IOException {
        ...
        for (int i = linkedTree.size() - 1; i >= 0; i--) {
            ...
            if (linkedTree.get(i).getStr().equals("SMOA")) {
                linkedTree.get(i).setValue(readWuMatrix.read("cmt-Conference-SMOA", 29, 59));
            }
            if (linkedTree.get(i).getStr().equals("wuAndPalmer")) {
                linkedTree.get(i).setValue(readWuMatrix.read("cmt-Conference-wuAndPalmer", 29, 59));
            }
            if (linkedTree.get(i).getStr().equals("sonGreedy")) {
                linkedTree.get(i).setValue(readWuMatrix.read("cmt-Conference-sonGreedy", 29, 59));
            }
