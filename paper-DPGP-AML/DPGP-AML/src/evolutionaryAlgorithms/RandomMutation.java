...
public class RandomMutation {
    ...
    public static List<ArrayList<LinkedTreeNode>> mutation(List<ArrayList<LinkedTreeNode>> populationWorse, int selectionSize, List<String> PSA, List<List<String>> source, List<List<String>> target) throws IOException {
        ...
        for (ArrayList<LinkedTreeNode> linkedTree : populationWorse) {
            ...
            if (r < 0.5) {
                int s;
                while (true) {
                    s = Double.valueOf(Math.random() * linkedTree.size()).intValue();
                    if (s != 0)
                        break;
                }
                ...
                ArrayList<LinkedTreeNode> mutationTree = mutationWay2(linkedTree, str1, str2, str3);
                populationUnion.add(mutationTree);
            }
        }
        List<ArrayList<LinkedTreeNode>> populationUpdate = DeterministicCrossover.roulette(populationUnion, selectionSize, Fitness.fitnessOfPopulation(populationUnion, PSA, source, target));
    ...
    public static ArrayList<LinkedTreeNode> mutationWay2(ArrayList<LinkedTreeNode> linkedTree, String[] str1, String[] str2, String[] str3) {
        List<ArrayList<LinkedTreeNode>> list = DeterministicCrossover.crossover(linkedTree, BT.createTree(str1, str2, str3, 3));
        return list.get(0);
    }
